...
\begin{enumerate}
    \item \textbf{Formal methods can be lightweight.} Automated inference removes the primary barrier to specification adoption: manual effort~\cite{zimmermann2019lightweight, hall1990seven}.

    \item \textbf{Specifications complement documentation.} Our tool infers specifications for undocumented methods, filling gaps that NLP-based approaches~\cite{jdoctor2018, pandita2012inferring} cannot address.
...
\begin{enumerate}
    \item \textbf{Enhanced loop invariant inference.} Machine learning approaches~\cite{wei2011inferring} and hybrid static-dynamic techniques~\cite{ernst2007daikon} may improve success rates beyond our current 85.3\%.

    \item \textbf{Object-oriented extensions.} Handling inheritance, polymorphism, and class invariants~\cite{meyer1992design, leavens2006design} would broaden applicability.
    ...
    \item \textbf{Interactive refinement.} Developer feedback could improve specification quality for complex methods~\cite{cousot2011contract}.

    \item \textbf{Runtime verification integration.} Using inferred specifications for runtime contract checking~\cite{meyer1992design} could detect violations in production.
\end{enumerate}
