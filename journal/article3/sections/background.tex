...
//@ ensures factor != 0 ==> factor != 0;   // tautology here
\end{lstlisting}
...
information the single pass cannot express. A second shape, the
polymorphic-dispatch disjunction, arises analogously when \texttt{x.n()} could
dispatch to several implementations; the sound caller precondition is the
disjunction \texttt{(p\_1 || p\_2 || \dots\ || p\_k)} of the candidates'
substituted preconditions, which a single nominal resolution cannot produce.
These two shapes are the structural subjects of Section~\ref{sec:empirical}.
