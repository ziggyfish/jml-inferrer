...
Our pass takes as input a populated \texttt{SpecificationCache}, a call graph over the same methods, and a map from method signature to the AST of each \texttt{MethodDeclaration}. We mutate the cache in place.
...
The two-tier treatment, and the iteration it entails, is forced by
the call graph rather than chosen for convenience. On an acyclic graph
a \emph{single} reverse-topological sweep already refines every method
against fully-refined callees: each callee precedes all of its callers,
so no clause it gains can invalidate a caller already visited, and one
pass suffices. A cycle breaks this guarantee. Within a mutually-recursive
SCC no order refines every callee before its caller, so a method first
visited against a still-stale callee spec must be revisited once that
callee is itself refined --- hence the repeat. Iterating to a fixpoint
is the minimal sound response: it terminates because each method's
clause set only grows and is bounded above by the finitely many
substituted callee clauses reachable in the SCC, and the cap merely
guards against pathological nesting depth, not against non-termination.
...
\begin{enumerate}\setlength\itemsep{0.2em}
  ...
  \item \textbf{Detect polymorphic dispatch.} If the resolved callee
        ...
        \texttt{(p\_1 || p\_2 || \dots\ || p\_k)}. Candidate
        enumeration is over-approximate (it never excludes a real
        dispatch target); the emitted disjunction is a deliberate
        \emph{weakening} of the caller obligation, not a sound safety
        guarantee (Section~\ref{sec:background}). A single nominal
        resolution cannot produce it at all.
