...
- the pass refines 23--43\% of methods per library (median 35.53\%) and adds
  45{,}740 cross-method precondition clauses the single-pass strategy cannot
  express;
- the addition is dominated by branch-conditional implications on four of
  five libraries (46--67\%) and includes polymorphic-dispatch disjunctions on
  every library (1.8--9.8\%) --- both shapes the single pass cannot produce
  by construction;
- the second pass is cheap: 34 seconds total for all 21{,}052 methods.
...
(3.12.0 vs 3.14.0) surfaces 525 candidate-breaking method-level changes and
144 strengthened preconditions --- exactly the artefact a behavioural-
