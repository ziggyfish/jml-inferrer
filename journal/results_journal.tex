...
Table~\ref{tab:jdoctor-comparison} compares specifications inferred by JML-Inferrer versus Jdoctor.
...
Unique to JML-Inferrer & 1,091 & --- \\
Unique to Jdoctor & --- & 735 \\
\bottomrule
\end{tabular}
\end{table}

JML-Inferrer infers specifications for 96.3\% of methods compared to Jdoctor's 45.0\%, as Jdoctor requires documented exceptions in Javadoc. However, the tools are complementary: Jdoctor captures 735 exception preconditions that JML-Inferrer missed (typically those with complex natural language descriptions), while JML-Inferrer captures 1,091 that Jdoctor missed (those not documented in Javadoc).
...
\begin{tabular}{lrr}
\toprule
\textbf{Metric} & \textbf{Static Analysis} & \textbf{LLM (Gemini 2.0)} \\
\midrule
...
We present test generation results at two levels: (1) detailed per-class results on the Apache Commons Lang subset (312 methods) for reproducibility, and (2) aggregated category-level results across the full OpenJDK dataset (10,709 methods) for generalizability.

\subsubsection{Per-Class Results (Apache Commons Lang)}

Table~\ref{tab:test-results-full} presents test generation results for the Apache Commons Lang subset, allowing detailed examination of specific classes.
...
\subsubsection{Category-Level Results (Full OpenJDK Dataset)}

Figure~\ref{fig:mutation-comparison} visualizes mutation scores aggregated by method category across the full 10,709 methods from OpenJDK. These results reveal important patterns not visible in the per-class analysis.
...
\end{axis}
\end{tikzpicture}
\caption{Mutation scores by category across the full OpenJDK dataset (10,709 methods). Error bars show standard deviation across 5 runs. Note that P3 (specifications) outperforms P4 (source code) in most categories.}
\label{fig:mutation-comparison}
\end{figure}

\paragraph{Statistical Analysis (Category-Level).}

Aggregating across all 10,709 methods by category reveals a striking finding:

\begin{itemize}
    \item \textbf{P3 vs. P1}: Mean mutation score improvement of 34.1 percentage points across categories, $p < 0.001$, $d = 2.87$ (very large effect).

    \item \textbf{P3 vs. P4}: Specification-guided generation (P3) achieves 3.5 pp \emph{higher} mutation scores than source-code-based generation (P4) when aggregated across categories (mean 92.1\% vs. 88.6\%). This counter-intuitive result suggests that formal specifications provide more focused testing guidance than raw implementation details, which can overwhelm the LLM with irrelevant complexity.

    \item \textbf{Per-class variation}: The Apache Commons Lang subset shows the opposite pattern (P4 > P3), likely due to the smaller sample size and the well-structured nature of these utility classes. The category-level aggregation across 10,709 diverse methods provides a more robust comparison.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{RQ1}: JML-Inferrer achieves 94.2\% precision and 89.3\% recall for preconditions, 87.6\% precision and 78.1\% recall for postconditions, validated through manual inspection of 500 specifications.

    \item \textbf{RQ2}: Specification-guided test generation produces 68.4\% more tests and achieves 34.1 pp higher mutation scores than signature-only baselines. Across the full 10,709-method dataset, specifications outperform source-code-based generation by 3.5 pp in mutation score.
