...
\paragraph{False positives.} Although precision is high (94.2\%), the 1.3\% of incorrect preconditions may mislead developers or cause spurious test failures. Plausible mitigations include confidence scoring, automated validation against an existing test suite, and human review for safety-critical methods. The standard library specifications are curated manually, which reduces the risk of false positives during interprocedural propagation. The optional OpenJML discharge step (Section~\ref{subsec:validation}) provides an additional safeguard: clauses that fail formal verification are flagged for review, catching residual unsoundness that manual inspection might miss, particularly in numeric corner cases such as integer-overflow boundaries and floating-point edge values.
