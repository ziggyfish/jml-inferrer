...
The same 312 methods are used both for the spec-accuracy validation and for the test-generation effectiveness comparison reported in subsequent sections. The two analyses are methodologically independent observations on the same population: the validation analysis compares inferred specifications against Javadoc-documented intent (does the spec match what the method should do?), while the effectiveness analysis compares LLM-generated tests across input conditions (does the spec, whatever its accuracy, change what the model produces?). No leakage between the two is possible: the LLM does not see the validation labels, and the validation labels are computed independently of the test-generation outcome.
