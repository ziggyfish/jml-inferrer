...
The 3\% fidelity gap reported in Section~\ref{sec:empirical} is an artefact of multi-release JAR keying, not a limit of the carrier. The follow-up release closes the gap. A more substantive limitation: roundtrip equality is on the canonical-form string, not on a normalised semantic theory, so equivalent specifications written in different forms will appear distinct on the wire. This is the deliberate separation-of-concerns chosen in Section~\ref{sec:format}; semantic normalisation is a separable layer.
