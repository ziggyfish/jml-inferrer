...
\begin{table}[ht]
\centering
\caption{Embedder validation on real-world libraries (medians across 39 runs for throughput; lossless-roundtrip rate and overhead are deterministic). The fourth row reports the full inferrer$\to$embedder pipeline on real inferrer-generated specifications; the synthetic specs in the first three rows are decoupled from the inferrer's behaviour to isolate the embedding mechanism. On matched methods, equality is exact (zero clause-level mismatches).}
...
\begin{table}[ht]
\centering
\caption{Spec-shape coverage on the 2{,}332 Commons Lang specs embedded through the real-inference harness (representative; Commons~IO and Guava give similar distributions). Every category roundtrips losslessly on every library.}
\label{tab:shapes}
\begin{tabular}{lrr}
\toprule
